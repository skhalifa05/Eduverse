...
<h3 style="padding-left: 22%;margin-top: 0%;color: #F0A61F;"><?php echo $center->getName(); ?></h3>
        ...
        <div style="margin-bottom: 50px" class="content">
            ...
            <div class="searches">
                ...
                <input type="text" id="myLevels" onkeyup="searchLevel()" placeholder="Search by Telephone.." title="Type in a level">
    ...
    <td class="icon-column">
        ...
        <div class="modal">
            <div style="margin-bottom: 10px" class="qrcode"></div>
        </div>
    </td>
                            <td>
                                <p>
                            ...
                            <td class="att-show" >
                                <button  onclick=" window.open('Viewh.php?Student_ID=<?php echo $row['Student_ID'];if (isset($_GET['Year'])){echo'&Year='.$_GET['Year'];}?>','_blank')">
                                    View
                                </button>
                            </td>
                            ...
                            <td class="att-show"  <?php if(!isset($_GET['Year'])){echo'style="display: none"';}?>>
                                <button  onclick="Duplicate(<?php echo $row['Student_ID'] ?>)">
                                    Duplicate
                                </button>
                            </td>
                            <td class="att-show"  <?php if(isset($_GET['Year'])){echo'style="display: none"';}?>>
...
window.addEventListener("load", () => {
    // Select all elements with the class 'qrcode' and generate QR code for each student ID
    document.querySelectorAll('.qrcode').forEach((qrcodeContainer, index) => {
        // Get the student ID from the data attribute of the parent row
        const studentId = qrcodeContainer.closest('tr').getAttribute('data-student-id');

        // Check if a valid student ID is available
        if (studentId) {
            const qrcodeElement = document.createElement("div");
            qrcodeElement.id = "qrcode" + index; // Unique ID for each QR code element
            qrcodeContainer.appendChild(qrcodeElement); // Append the QR code element to the modal
    ...
    function Duplicate(id) {
        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                myDiv.style.cssText = 'color:#fff';
                //Add your content to the DIV
                myDiv.innerHTML = "Student added!";

                //Finally, append the element to the HTML body
                document.body.appendChild(myDiv);
            }
        };
        xhttp.open("GET", `https://brightside-edu.com/Controller/Student_Handling.php?Student_ID=${id}&Duplicate&Year=<?php echo $_GET['Year']?>`, false);
        xhttp.send();
    }
