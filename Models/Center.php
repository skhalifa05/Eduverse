...
class Center
{
    ...
    public function expect(mysqli $conn, string $day):int
    {
        ...
        while($row = $Courses->fetch_assoc()){
            $stmt = $conn->prepare("SELECT COUNT(DISTINCT time_table.User_ID) AS Enroll_Count FROM courses
            LEFT JOIN course_time ON course_time.Course_ID = courses.Course_ID
            LEFT JOIN time_table ON course_time.Course_Time_ID = time_table.Course_Time_ID
            WHERE courses.Course_ID = ?
            GROUP BY courses.Course_ID;");
     ...
     * @return mysqli_result
     */
    public function ReturnOldCourses(mysqli $conn, $year):mysqli_result
    {
        $stmt = $conn->prepare("SELECT * FROM ".$year."_courses");
        $stmt->execute();
        return $stmt->get_result();
    }
    ...
    public function GetOldDoctorName(mysqli $conn, int $course_id, $year):String
    {
        $stmt = $conn->prepare("SELECT doctor.F_Name,doctor.L_Name, doctor.M_Name FROM ".$year."_courses
            LEFT JOIN doctor ON doctor.Admin_ID = ".$year."_courses.Admin_ID
            WHERE ".$year."_courses.Course_ID = ?");
        $stmt->bind_param("i", $course_id);
        $stmt->execute();
        $stmt = $stmt->get_result();
        $stmt = $stmt->fetch_assoc();
    ...
    public function GetOldUsersEnrolledCount(mysqli $conn, int $course_id, $year):int
    {
        $stmt = $conn->prepare("SELECT COUNT(".$year."_time_table.User_ID) AS Enroll_Count FROM ".$year."_courses
            LEFT JOIN ".$year."_course_time ON ".$year."_course_time.Course_ID = ".$year."_courses.Course_ID
            LEFT JOIN ".$year."_time_table ON ".$year."_course_time.Course_Time_ID = ".$year."_time_table.Course_Time_ID
            WHERE ".$year."_courses.Course_ID = ?
            GROUP BY ".$year."_courses.Course_ID;");
        $stmt->bind_param("i", $course_id);
        $stmt->execute();
        $stmt = $stmt->get_result();
        if($stmt->num_rows === 0){
    ...
    public function DeleteStudentRequest(mysqli $conn, int $reuqest_id)
    {
        $stmt = $conn->prepare("DELETE FROM requests WHERE requests.Course_ID = ?");
        $stmt->bind_param("i", $reuqest_id);
        $stmt->execute();
    }
    ...
    public function Enablecourse(mysqli $conn, int $courseID)
    {
        $stmt = $conn->prepare("UPDATE courses SET stat = '1' WHERE courses.Course_ID = ?");
        $stmt->bind_param("i", $courseID);
        $stmt->execute();
    }
    ...
    {
        $stmt = $conn->prepare("SELECT DISTINCT COUNT(time_table.User_ID) AS Enroll_Count FROM course_time
            LEFT JOIN time_table ON course_time.Course_Time_ID = time_table.Course_Time_ID
    ...
    public function ReturnAllOldLecturesFinished(mysqli $conn, $year):mysqli_result
    {
        $stmt = $conn->prepare("SELECT ".$year."_lectures.Lecture_ID, ".$year."_lectures.State, ".$year."_lectures.Name AS L_NAme, ".$year."_courses.Name AS C_Name, doctor.F_Name, doctor.L_Name, doctor.M_Name FROM ".$year."_lectures
            LEFT JOIN ".$year."_course_time ON ".$year."_course_time.Course_Time_ID = ".$year."_lectures.Course_Time_ID
            LEFT JOIN ".$year."_courses ON ".$year."_course_time.Course_ID = ".$year."_courses.Course_ID
            LEFT JOIN doctor ON doctor.Admin_ID = ".$year."_courses.Admin_ID WHERE ".$year."_lectures.State = 'FINISHED' or ".$year."_lectures.State = 'Cancelled'
            ORDER BY ".$year."_lectures.Lecture_ID DESC;");
        $stmt->execute();
        return $stmt->get_result();
    }
    ...
    {
        $stmt = $conn->prepare("SELECT DISTINCT student.Student_ID, student.F_Name, student.L_Name, student.M_Name FROM course_time
    ...
    public function GetOldStudentListForCourseByID(mysqli $conn, int $courseid, $year):mysqli_result
    {
        $stmt = $conn->prepare("SELECT ".$year."_students.Student_ID, ".$year."_students.F_Name, ".$year."_students.L_Name, ".$year."_students.M_Name FROM ".$year."_course_time
            LEFT JOIN ".$year."_time_table ON ".$year."_time_table.Course_Time_ID = ".$year."_course_time.Course_Time_ID
            LEFT JOIN ".$year."_students ON ".$year."_students.Student_ID = ".$year."_time_table.User_ID
            WHERE ".$year."_course_time.Course_ID = ? 
            ORDER BY ".$year."_students.Student_ID");
        $stmt->bind_param("i", $courseid);
        $stmt->execute();
        return $stmt->get_result();
    }
    ...
    public function getOldCNameforVS(mysqli $conn, int $courseid, $year):mysqli_result
    {
        $stmt = $conn->prepare("SELECT Name FROM ".$year."_courses WHERE Course_ID = ?");
        $stmt->bind_param("i", $courseid);
        $stmt->execute();
        return $stmt->get_result();
    }
    ...
    {
        $stmt = $conn->prepare("SELECT Course_Time_ID FROM course_time WHERE Course_ID = ? LIMIT 1");
        $stmt->bind_param("i", $courseid);
        $stmt->execute();
        return $stmt->get_result();
    }
    ...
    public function GetOldAttendanceListForLectureByID(mysqli $conn, int $lecture_id, $year):mysqli_result
    {
...
public function OldExcelAttendanceReport(mysqli $conn, int $lecture_id, $year)
{
    $stmt = $conn->prepare("SELECT ".$year."_attendance_list.State, ".$year."_attendance_list.Arrival_Time, ".$year."_students.F_Name, ".$year."_students.L_Name, ".$year."_students.M_Name, ".$year."_lectures.Name AS L_Name, ".$year."_courses.Name AS C_Name, ".$year."_lectures.Lecture_ID, ".$year."_students.Student_ID FROM ".$year."_attendance_list
        LEFT JOIN ".$year."_students ON ".$year."_students.Student_ID = ".$year."_attendance_list.User_ID
        LEFT JOIN ".$year."_lectures ON ".$year."_lectures.Lecture_ID = ".$year."_attendance_list.Lecture_ID
    ...
    public function GetOldUserCountsForLecture(mysqli $conn, int $lecture_id, $year):mysqli_result
    {
        $stmt = $conn->prepare("SELECT COUNT(".$year."_attendance_list.User_ID) AS Counter, ".$year."_attendance_list.State FROM ".$year."_attendance_list
            WHERE ".$year."_attendance_list.Lecture_ID = ?
            GROUP BY ".$year."_attendance_list.State;");
        $stmt->bind_param("i", $lecture_id);
        $stmt->execute();
        return $stmt->get_result();
    }
    ...
    public function AddOLDStudent(mysqli $conn, int $id, String $f_name, String $m_name, String $l_name, String $email, String $password, String $Phone, String $school_name, int $grade, int $age, String $relation_name, String $parent_name, String $relation_phone, int $Veri)
    {
        $stmt = $conn->prepare("INSERT INTO student (Student_ID, F_Name, M_Name, L_Name, Email, Password, Phone, School_Name, Grade, Age, Relation_Name, Parent_Name, Relation_Phone, Verify) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isssssssiisssi",$id, $f_name, $m_name, $l_name, $email, $password, $Phone, $school_name, $grade, $age, $relation_name, $parent_name, $relation_phone, $Veri);
        $stmt->execute();
    }
     ...
     * @return mysqli_result
     */
    public function GetAllRequests(mysqli $conn):mysqli_result
    {
     ...
     * @return mysqli_result
     */
    public function GetArchivedStudents(mysqli $conn, $Year):mysqli_result
    {
        $stmt = $conn->prepare("SELECT * FROM ".$Year."_students");
        $stmt->execute();
        return $stmt->get_result();
    }
    ...
    public function deleteRequest(mysqli $conn, $reqid)
    {
        $update = $conn->prepare("DELETE FROM requests WHERE requests.Student_ID = ?");
     ...
     * @return array
     */
    public function GetstudentRequest(mysqli $conn, $student_id):array
    {
        $stmt = $conn->prepare("SELECT * FROM requests WHERE requests.Student_ID = ?;");
        $stmt->bind_param("i",$student_id);
        $stmt->execute();
        $results = $stmt->get_result();
        return $results->fetch_assoc();
    ...
    {
//        update next seesion with this new sql statement, which selects according to the course
//        time id so the user can enroll in the same course but multiple timings instead of creating multiple courses
//        for diff timings, add parameter for eligible times user can enroll (sessions/week):
//            SELECT `course_time`.`Course_Time_ID` FROM `course_time` WHERE `course_time`.`Course_Time_ID` = '64'
//            AND `course_time`.`Course_Time_ID` IN
//            (SELECT `time_table`.`Course_Time_ID` FROM `time_table` WHERE `time_table`.`User_ID` = '20220454');
        $stmt = $conn->prepare("SELECT course_time.Course_Time_ID FROM course_time WHERE course_time.Course_Time_ID = ?
            AND course_time.Course_Time_ID IN
            (SELECT time_table.Course_Time_ID FROM time_table WHERE time_table.User_ID = ?)");
    ...
    public function RetSOldCourse(mysqli $conn, int $stuID, $year):mysqli_result
    {
        $stmt = $conn->prepare("SELECT ".$year."_courses.Course_ID, ".$year."_courses.Name FROM ".$year."_time_table
            LEFT JOIN ".$year."_course_time ON ".$year."_course_time.Course_Time_ID = ".$year."_time_table.Course_Time_ID
            LEFT JOIN ".$year."_courses ON ".$year."_courses.Course_ID = ".$year."_course_time.Course_ID WHERE ".$year."_time_table.User_ID = ?;");
        $stmt->bind_param("i", $stuID);
        $stmt->execute();
        return $stmt->get_result();
    }
    ...
    public function GetsOldAttlect(mysqli $conn, int $stuID, int $courseid, $year):mysqli_result
    {
        $stmt = $conn->prepare("SELECT COUNT(".$year."_attendance_list.User_ID) AS Counter, ".$year."_attendance_list.State FROM ".$year."_attendance_list
            LEFT JOIN ".$year."_lectures ON ".$year."_attendance_list.Lecture_ID = ".$year."_lectures.Lecture_ID
            LEFT JOIN ".$year."_course_time ct ON ".$year."_lectures.Course_Time_ID = ct.Course_Time_ID 
            LEFT JOIN ".$year."_courses c ON c.Course_ID = ct.Course_ID
            WHERE ".$year."_attendance_list.User_ID = ? AND c.Course_ID = ?
            GROUP BY ".$year."_attendance_list.State;");
        $stmt->bind_param("ii", $stuID, $courseid);
        $stmt->execute();
        return $stmt->get_result();
    ...
    public function RetOldCLect(mysqli $conn, int $stuID, int $courseID, $year):mysqli_result
    {
        $stmt = $conn->prepare("SELECT ".$year."_lectures.Name, ct.Day, ct.Time, ".$year."_attendance_list.State,".$year."_attendance_list.Arrival_Time FROM ".$year."_attendance_list
            LEFT JOIN ".$year."_lectures ON ".$year."_attendance_list.Lecture_ID = ".$year."_lectures.Lecture_ID
            LEFT JOIN ".$year."_course_time ct ON ".$year."_lectures.Course_Time_ID = ct.Course_Time_ID 
            LEFT JOIN ".$year."_courses c ON c.Course_ID = ct.Course_ID
            WHERE ".$year."_attendance_list.User_ID = ? AND c.Course_ID = ?;");
    ...
    public function GetOldsName(mysqli $conn, int $stuid, $year):mysqli_result
    {
        $stmt = $conn->prepare("SELECT F_Name, M_Name, L_Name, Email, Password, Phone, School_Name, Grade, Age, Relation_Name, Parent_Name, Relation_Phone, Verify FROM ".$year."_students
            WHERE ".$year."_students.Student_ID = ?;");
        $stmt->bind_param("i", $stuid);
        $stmt->execute();
        return $stmt->get_result();
    }
     ...
     * @return int
     */
    public function archstucount(mysqli $conn, $Year):int
    {
        $stmt = $conn->prepare("SELECT * FROM ".$Year."_students");
        $stmt->execute();
        return mysqli_num_rows($stmt->get_result());
    ...
    public function GetExcessiveAbsence(mysqli $conn):mysqli_result
    {
        $stmt = $conn->prepare("SELECT s.Student_ID, s.F_Name, s.M_Name, s.L_Name, s.Phone, s.Relation_Phone, c.Name as Course_Name, COUNT(al.User_ID) as Absence_Count
FROM student s
LEFT JOIN attendance_list al ON s.Student_ID = al.User_ID
LEFT JOIN lectures l ON al.Lecture_ID = l.Lecture_ID
LEFT JOIN course_time ct ON l.Course_Time_ID = ct.Course_Time_ID
LEFT JOIN courses c ON ct.Course_ID = c.Course_ID
WHERE al.State = 'absent' AND c.stat = 1
GROUP BY s.Student_ID, ct.Course_ID
HAVING Absence_Count > 3 ORDER BY Absence_Count DESC;");
        $stmt->execute();
        return $stmt->get_result();
    }
}